...
```{r}
spec <- spec %>%
    filter(SiteStatus %in% c("control", "mature") & NetPan == "net" & Sex == "f")
...
spec <- spec %>%
    select(c("UniqueID", "GenusSpecies", "Site", "SiteStatus", "Year"))
...
### your code here
traits <- traits %>%
    select(c("GenusSpecies", "MeanITD", "d"))

spec.traits <- spec %>%
    left_join(traits, by = "GenusSpecies")
...
```{r}
### your code here
unique(spec.traits[is.na(spec.traits$MeanITD), "GenusSpecies"])
```
...
sys.traits <- spec.traits %>%
    ...
    summarize(MeanITD = mean(MeanITD),
              d = mean(d))
...
ggplot(sys.traits, aes(x = MeanITD)) +
    geom_histogram(bins = 20) +
    labs(x = "average body size")
```
...
ggplot(sys.traits, aes(x = d)) +
    geom_histogram(bins = 20) +
    labs(x = "average specialization")
```
- 3c. Describe what you see.

ANSWER: The average body size of bees across all combinations of site, year,
and site status is centered on 1.2 with outliers out towards 1.6 and 2. The 
average floral specialization is centered on 0.15 with outliers towards 0.20 and 0.25.
...
randomizeTraits <- function(yr, sys.traits) {
    # Takes a variable yr and the dataset sys.traits
    # Shuffles the dataset by yr

    year <- sys.traits %>%
        filter(Year == yr)
    
    year.shuffle <- sample(year$SiteStatus,
                           nrow(year), replace = FALSE)

    year$ShuffleSiteStatus <- year.shuffle
...
```
-- ANSWER: The values do not change across multiple uses as the seed is set at
the beginning of the document. This seed set up reproducible values and therefore
set the values to not change.
...
```{r}
random.traits <- data.frame(randomizeTraits(years[1], sys.traits))
for (year in years[-1]) {
    random_year <- randomizeTraits(year, sys.traits)
    random.traits <- rbind(random.traits, random_year)
}
random.traits
```
...
```{r, message=FALSE}
randomized.comms <- data.frame(repRandomComm(years, sys.traits))
for (i in 1:99) {
...
```{r}
## your code here
ggplot(randomized.comms, aes(x = d, fill = ShuffleSiteStatus)) +
    geom_histogram() +
    geom_vline(obs.means,
               mapping = aes(xintercept = d, linetype = SiteStatus)) +
    labs(x = "average specialization")
...
```{r}
### your code here
randomized.standard <- randomized.comms %>%
  filter(ShuffleSiteStatus == "control") %>%
  select(d)

obs.standard <- obs.means %>%
  filter(SiteStatus == "control") %>%
  select(d)

sum(randomized.standard$d <= obs.standard$d) / nrow(randomized.standard)
```

- 6c. Do we accept or reject our null hypothesis? 
-- ANSWER: Reject the null hypothesis as the p-value is less than 0.05
